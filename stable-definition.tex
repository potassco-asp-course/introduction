% ----------------------------------------------------------------------
\begin{frame}{\alt<8->{Stable models}{What is the meaning of a logic program?
      \\\phantom{S}\only<6-8>{\alert{Stable models!}}}}

  \begin{itemize}
  \item <2-13> The \alert{reduct}, \reduct{P}{X}, of a program $P$ relative to
    a set $X$ of atoms is
    \begin{align*}
      \reduct{P}{X} &= \{\head{r}\leftarrow\pbody{r} \mid r\in P,\ \nbody{r}\cap X=\emptyset\}
    \end{align*}

  \item<only@3-4> \structure{Note} \ \reduct{P}{X} can be obtained from $P$ by
    \begin{enumerate}\normalsize
  ...
  \item<only@9-10> \structure{Note}
    \begin{itemize}\normalsize
    \item $\Cn{\reduct{P}{X}}$ is the $\subseteq$-smallest \alt<10>{model of}{set closed under} \reduct{P}{X}
      \smallskip
    \item Each atom in $X$ is justified by a proof from \reduct{P}{X}
    \end{itemize}

  \item<only@11-12> \structure{Procedural counterpart}
