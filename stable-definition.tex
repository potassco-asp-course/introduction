...
\begin{frame}{This is a stable model!}{Stable models of normal programs}
  ...
  \item <3-> A set $X$ of atoms is a \alert{stable model} of a program $P$,
    ...
    \bigskip
    \bigskip
  \item<4-> \structure{Remarks}
    \begin{itemize}\normalsize
    \item<4-> $\Cn{\reduct{P}{X}}$ is the $\subseteq$--smallest (classical) model of \reduct{P}{X}
      \medskip
    \item<5-> Each atom in $X$ is justified by an \emph{``applying rule from $P$''}
      % \item<5-> Set $X$ is \alert{stable} under \emph{``applying rules from $P$''}
    \end{itemize}
  \end{itemize}
\end{frame}
% ----------------------------------------------------------------------
