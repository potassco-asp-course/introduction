...
\begin{frame}{What is the meaning of a \alt<1>{logic program}{set~of\only<3->{~positive}~rules}?}
  ...
  \item<only@8-27> [] \only<9->{\textbf{let} $\T{P}{X}=\{\head{r}\mid r\in P, \pbody{r}\subseteq X\}$ \textbf{in}} \
    ...
    \item<only@10-26> \structure{Example} \
      ...
      \end{tabular}
    \item<only@27> \structure{Procedural answer} \ The ``meaning of $P$'' is given by\par the value $X$ returned by the procedure

  \item<only@28-57> \structure{Mathematical characterization}
    \begin{itemize}\normalsize
    \item<only@29-57> A set $X$ of atoms is \alert{closed} under a positive program $P$ if
      \par\smallskip
    ...
    \item<only@56-57> \structure{Mathematical answer} \ The ``meaning of $P$'' is given by
      \par the \alert{smallest} set of atoms closed under $P$
      \smallskip
    \item<only@57> and denoted by \Cn{P}\pause[58], and called the \alert{consequences} of $P$
    \end{itemize}
