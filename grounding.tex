...
\begin{frame}{Ground instantiation}
  ...
  \begin{itemize}
  \item<1-> Let $\mathcal{T}$ be a set of \invisible<2-3>{(}variable-free\invisible<2-3>{)} \alert<1-2>{terms}
    \only<2  >{\\ (also called                           \alert{Herbrand universe})}
  \item<1-> Let $\mathcal{A}$ be a set of \invisible<2-3>{(}variable-free\invisible<2-3>{)} \alert<1-2>{atoms} constructible from $\mathcal{T}$
    \only<2-3>{\\ (also called \alt<3>{\alert{alphabet}}{\alert{Herbrand base}})}
    \smallskip
  \item<4-> A variable-free atom is also called \alert{ground}
    \medskip
  \item<5-> \structure{Ground instances} of $r\in P$\,: Set of variable-free rules obtained by
    ...
    \medskip
  \item<6-> \structure{Ground instantiation} of $P$\,: \
    \(
